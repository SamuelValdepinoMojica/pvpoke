...
# Custom Environment and Masked Policy imports
from environment.ClassPVPOKE import PVPokeEnv
from rl.masked_a2c import MaskedA2CPolicy
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train Stable Baselines 3 Masked A2C for PvPoke.")
    ...
    parser.add_argument("--n-steps", type=int, default=10, help="Steps to run per update")
    ...
    parser.add_argument("--run-name", default="a2c_run", help="Name of the experiment run")
    parser.add_argument("--output-dir", default=str(ROOT_DIR / "outputs" / "a2c"), help="Output directory")
...
def make_env(ws_uri, client_id, target_id, format, obs_mode):
    """Instantiates the base environment and applies Monitor + Wrapper."""
    base_env = PVPokeEnv(
        ws_uri, 
        client_id, 
        target_id, 
...
def main() -> None:
    ...
    logging.info(f"Starting A2C Training ({args.total_timesteps} steps)")
    ...
    try:
        # 2. Build the A2C Model passing our Custom Masked Policy
        model = A2C(
        ...
        if eval_freq > 0:
            eval_callback = EvalCallback(
                env, # Using the SAME exact environment as requested
        ...
        final_path = os.path.join(model_dir, "a2c_final_model")
        model.save(final_path)
        logging.info(f"Training finished in {duration/60:.2f} minutes. Model saved to {final_path}")

        # 6. Post-Training Evaluation (Reuses the exact same environment connection)
